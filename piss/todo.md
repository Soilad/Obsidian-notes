fuckboys sending u a photo of two cats 
'aww look this is us' 
(he has sent the same
type of image to 27 girls in his lifetime)

dhudhudhao dhudhudhudhudhdhtao

55cb5c84fd8ff500c61996a5559c9aa4f93c14e8618e624e1b0010b361f00df0

#### uhhh

#### fucking whatever
- [ ] texture province to biome
- [ ] add characters frm interpretermanager
- [ ] promote on youtub
- [ ] civil wars
- [ ] focus editor
- [ ] add filips focus trees
#### resources make work good
- [x] resource tags => literal fucking chemistry
- [x] designing (kinda like hoi4 division templates)
	 - [x] Power, Water, Waste, People (limit)
- [ ] actually store all of these in countries
- [ ] building types 
	- [ ]  oil rigs
	- [ ]  mines
	- [ ]  crafts
	- [ ]  supply?
	- [ ]  food - famine
	- [ ]  genocide???
	- [ ]  radar
	- [ ]  air defence
	- [ ]  unis
	- [ ]  town hall???
	- [ ]  market
	- [x]  hospital
	- [ ]  points of interests
		- [ ] tourism & tax havens
- [ ] add factories
	 - [ ] equipment
		  - [ ] bombs
		  - [ ] nukes

#### imp figures
- [ ] assassination
- [ ] research
- [ ] state owners??
- [ ] espionage
	 - [ ] instilling govs
	 - [ ] diplomacy stuff
- [ ] generals/field marshals
	 - [ ] template troops
		  - [ ] mercenary
		  - [ ] race hiring - slave armies??
		  - [ ] tasks (moddable) like kill ppl burn shit fuck school
	 - [ ] airforce??
	 - [ ] navy?
#### achievements
- [ ] binky bonky boo (get embargoed by every country)



add commands to yuchiru

piper-tts -m piper-models/ en_US-kristin-medium/ en_US-kristin-medium.onnx --output_raw | aplay -r 22050

find window with search

  the input types can also be bools